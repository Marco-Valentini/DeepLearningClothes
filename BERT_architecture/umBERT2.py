...
class umBERT2(nn.Module):
    ...
    def __init__(self, catalogue_sizes: dict, d_model=512, num_encoders=6, num_heads=1, dropout=0, dim_feedforward=None):
        ...
        self.catalogue_sizes = catalogue_sizes  # number of items in the catalogue
        ...
        self.ffnn_bottoms = nn.Linear(d_model, catalogue_sizes['bottoms'])  # for prediction of the fourth item in the outfit
    ...
    def forward(self, outfit):
        ...
        logits_shoes = self.ffnn_shoes(output[:, 1, :])  # compute the logits for the first item in the outfit
        logits_tops = self.ffnn_tops(output[:, 2, :])  # compute the logits for the second item in the outfit
        logits_acc = self.ffnn_acc(output[:, 3, :])  # compute the logits for the third item in the outfit
        logits_bottoms = self.ffnn_bottoms(output[:, 4, :])  # compute the logits for the fourth item in the outfit

        return {'clf': clf,
                'shoes': logits_shoes,
                'tops': logits_tops,
                'accessories': logits_acc,
                'bottoms': logits_bottoms}
