...
class umBERT2_trainer():
    ...
    def pre_train(self, datasets, run):
        """
        This function performs the pre-training of the umBERT model in a BERT-like fashion (MLM + classification tasks)
        :param datasets: the dataset containing the data and the labels (train and validation)
        ...
        for epoch in range(self.n_epochs):
            for phase in ['train', 'val']:
                ...
                # we are working in full batch mode (no mini-batches)
                inputs = datasets[phase][0]  # get the inputs of the phase
                labels = datasets[phase][1]  # get the labels of the phase
                ...
                # update the loss value (we are working in full batch mode)
                running_loss += loss.item() * inputs.size(0)

                # update the accuracy of the classification task
                pred_labels_CLF = self.model.sigmoid(dict_outputs['clf'])
                ...
                epoch_loss = running_loss / len(datasets[phase].dataset)  # compute the average loss of the epoch
                # compute the average accuracy of the classification task of the epoch
                epoch_accuracy_CLF = accuracy_CLF / len(datasets[phase].dataset)
                # compute the average accuracy of the items classification task of the epoch
                epoch_accuracy_shoes = accuracy_shoes / len(datasets[phase].dataset)
                epoch_accuracy_tops = accuracy_tops / len(datasets[phase].dataset)
                epoch_accuracy_acc = accuracy_acc / len(datasets[phase].dataset)
                epoch_accuracy_bottoms = accuracy_bottoms / len(datasets[phase].dataset)
                ...
                if phase == 'train':
                    ...
                    if epoch_loss <= valid_loss_min:
                        ...
                        torch.save(checkpoint,
                                   f'../models/umBERT3_pre_trained_{self.model.d_model}.pth')  # save the checkpoint dictionary to a file
    ...
    def fine_tuning(self, datasets, run=None):
        """
        Fine-tuning of the model on the fill in the blank task
        :param datasets: the datasets for the training and validation phases (dict)
        ...
        for epoch in range(self.n_epochs):
            for phase in ['train', 'val']:
                ...
                # we are working in full batch mode, so we don't need to iterate over the batches
                inputs = datasets[phase]  # get the inputs of the phase
                labels = datasets[phase]  # get the labels of the phase
                ...
                # compute the average accuracy of the MLM task of the epoch
                epoch_accuracy_MLM = accuracy_MLM / len(datasets[phase].dataset)
                ...
                if phase == 'train':
                    ...
                    if epoch_loss <= valid_loss_min:
                        ...
                        torch.save(checkpoint,
                                   f'../models/umBERT3_finetuned_{self.model.d_model}.pth')  # save the checkpoint dictionary to a file
